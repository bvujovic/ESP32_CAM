//* ESP32CAM je podesen kao AP WiFi i strimuje sliku na 192.168.4.1:81/stream
//* Potrosnja struje je nesto ispod 200mA.
/*
  ESP32CAM Robot Car
...
void startCameraServer()
{
    ...
    config.server_port += 1; // podesavanje za port 81
    config.ctrl_port += 1;
...
void setup()
{
    ...
    Serial.printf("Camera init res: %d\n", res);

    // Connect to Wi-Fi network with SSID and password
    Serial.print("Setting AP (Access Point)...");
    // Remove the password parameter, if you want the AP (Access Point) to be open
    WiFi.softAP("ESP32CAM-AP");
